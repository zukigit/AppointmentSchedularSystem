...
@Component({
    selector: 'app-traineenavbar',
    templateUrl: './traineenavbar.component.html',
    styleUrls: ['./traineenavbar.component.scss']
  })
  export class TraineenavbarComponent implements OnInit {
    ...
    goAppDetail(appId:Number, notiId:number, type:string, deletedDate:string, title:string) {
        if(type == "DELETE_APP") {
            ...
            this.router.navigate(['/trainee/appointment_detail_view_bytrainee', appId]).then (() => window.location.reload()),10000;
        }
    }
